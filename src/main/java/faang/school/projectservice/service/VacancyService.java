...
public class VacancyService {
    ...
    private void attachCandidates(VacancyDto vacancyDto, Vacancy createdVacancy) {
        vacancyDto.getCandidateIds().forEach((candidateId) -> {
            Candidate candidateToEdit = candidateRepository.findById(candidateId)
                    .orElseThrow(() -> new EntityNotFoundException("Candidate not found"));
            candidateToEdit.setVacancy(createdVacancy);
    ...
    public Map<String, String> update(Long id, VacancyDto vacancyDto) {
        ...
        if (vacancyDto.getStatus().equals(VacancyStatus.CLOSED) && editVacancy.getCount() > editVacancy.getCandidates().size()) {
    ...
    private void deleteVacancy(Vacancy vacancyToDelete) {
        List<Long> candidatesToDelete = vacancyToDelete.getCandidates().stream().filter(el -> !el.getCandidateStatus().equals(CandidateStatus.ACCEPTED)).map(Candidate::getId).toList();

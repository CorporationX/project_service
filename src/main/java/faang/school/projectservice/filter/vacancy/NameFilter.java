...
@Component
public class NameFilter implements Filter<VacancyFilterDto, Vacancy> {
    ...
    public Stream<Vacancy> applyFilter(Stream<Vacancy> vacancies, VacancyFilterDto vacancyFilterDto) {
        return vacancies.filter(vacancy -> vacancy.getName().equalsIgnoreCase(vacancyFilterDto.getName()));
    }
}

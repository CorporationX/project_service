...
public class MeetDateFilter implements MeetFilter {
    ...
    public boolean isApplicable(MeetFilterDto filters) {
        return filters.getStartDateTime() != null && filters.getEndDateTime() != null;
    }

    @Override
    public Stream<Meet> apply(Supplier<Stream<Meet>> meetings, MeetFilterDto filters) {
        return meetings.get()
                .filter(meet -> meet.getStartDateTime().isAfter(filters.getStartDateTime())
                        && meet.getEndDateTime().isBefore(filters.getEndDateTime()));
    }
}
